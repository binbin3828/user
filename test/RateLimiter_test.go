...
func TestRateLimiter_AllowsWithinLimit(t *testing.T) {
	rl := service.NewRateLimiter(5, time.Minute)
...
func TestRateLimiter_BlocksExceeded(t *testing.T) {
	rl := service.NewRateLimiter(2, time.Minute)
...
func TestRateLimiter_DifferentIPsIndependent(t *testing.T) {
	rl := service.NewRateLimiter(1, time.Minute)
...
func TestRateLimiter_ResetsAfterWindow(t *testing.T) {
	rl := service.NewRateLimiter(1, 50*time.Millisecond)
...
func TestRateLimitMiddleware_DoesNotCrash(t *testing.T) {
	rl := service.NewRateLimiter(100, time.Minute)
